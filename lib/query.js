...
module.exports = function Moomyquery(model, filter) {
	...
	myq.query = {
		...
		startAt: false,
		filters: {},
		filtersSift: {}, // Additional Sift criteria to filter by - only works if myq.soft.query is enabled, which is enabled via the `.sift()` wrapper
		select: [],
		sort: [],
		flatten: false,
		lean: false,
	...
	*/
	...
	/**
	* Append Sift query criteria
	* This also enables soft.query
	* @param {Object|boolean} [filter...] The sift filters to apply, if the only parameter is a boolean soft quering is enabled or disabled by its value, if its an object soft quering is enabled AND the object is used as the query
	* @returns {MoodyQuery} This chainable query
	*/
	myq.sift = (...filter) => {
		if (filter.length == 1 && _.isBoolean(filter[0])) {
			myq.query.soft.query = filter[0];
		} else {
			myq.query.soft.query = true;
			_.merge(myq.query.filtersSift, ...filter);
		}
		return myq;
	};
	...
	*/
	...
	/**
	* Set a starting index for the query to begin at
	* NOTE: Dynamo only accepts keys in dynamo form i.e. {id: S: {'***********'}}
	* @returns {MoodyQuery} This chainable query
	*/
	myq.startAt = index => {
		myq.query.startAt = index;
		return myq;
	};

	/**
	* Set that we want responses exceeding Dynamo's 1MB response limit, this will repeatedly query via lastKey callback
	* @returns {MoodyQuery} This chainable query
	*/
	myq.all = ()=> {
		myq.query.all = true;
	...
	*/
	myq.exec = myq.promise = ()=> Promise.resolve()
		...
		.then(()=> {
			if (
				...
				return new Promise((resolve, reject) => {
					...
					if (myq.query.startAt) q.startAt(myq.query.startAt)
					...
					q.exec(async(err, res) => {
						...
						if (err) return reject(err);
								if(res.lastKey && (!myq.query.limit || myq.query.limit >= res.length || myq.query.all)) {
									myq.query.startAt = res.lastKey;
									await myq.exec()
										.then(moreData => res = res.concat(moreData))
										.then(res => res.length > myq.query.limit ? res.length = myq.query.limit : res)
								}
								if (myq.query.count) {
									var count = res[0] || 0;
									debugDetail('Counted', count, 'docs from query');
									resolve(count);
								} else {
				...
				return new Promise((resolve, reject) => {
					...
					if (myq.query.all) console.log('registering')
					if (myq.query.count) q.count();
					if (myq.query.limit) q.limit(myq.query.limit);
					if (myq.query.index) q.using(myq.query.index);
					if (myq.query.selected) q.attributes(myq.query.selected);
		...
		// }}}
		// Soft query (suplement the "hard" DB query with Sift) {{{
		.then(res => {
			if (!myq.query.soft.query || !_.isArray(res)) {
				return res; // Not an enabled or a filterable array anyway
			} else {
				var siftQuery = {...myq.query.filters, ...myq.query.filtersSift};
				debugDetail('Applying sift query', siftQuery);
				return res.filter(sift(siftQuery));
			}
		})
		// }}}
		// Soft select {{{
