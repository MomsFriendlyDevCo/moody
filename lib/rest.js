...
* @param {boolean} [options.softQuery=false] Enable Sift powered soft querying (slow but more accurate than Dynamo)
			...
			.then(()=> {
				...
				if (middleware === true || (Array.isArray(middleware) && !middleware.length)) { // Endpoint enabled or no middleware to call
					...
					return new Promise((resolve, reject) => {
						var middlewareOffset = 0;
						var runNextMiddleware = err => {
							if (err) return reject(err);
							var thisMiddleware = middleware[middlewareOffset++];
