...
module.exports = function(dy, id, schema) {
	...
	dym.prototype = {};
	dym.virtuals = {}; // Lookup table for virtuals (corresponds with Object.defineProperties spec)
	...
	*/
	dym.createMany = docs => {
		debug('Create', docs.length, 'docs in batch. Will submit in', Math.ceil(docs.length / 100), 'batches of', dy.settings.createMany.batchSize, 'documents');
	...
	* @returns {DynamooseModel} This chainable model
	*/
	dym.virtual = (name, getFunc, setFunc) => {
		dym.virtuals[name] = {
			get: getFunc,
			set: setFunc,
		};

		debug(`Alloc virtual method "${name}" to ${dym.id}`)
