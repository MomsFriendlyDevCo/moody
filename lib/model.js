...
module.exports = function(my, id, schema, options) {
	...
	mym.prototype = {
		...
		*/
		$eachDocumentNode(path, func, context = this, currentPath = []) {
			var rootDoc = this;
			if (!_.isArray(path)) path = path.split('.');
			...
			Object.keys(mym.valuePaths).forEach(vPath => {
				waitingOn = waitingOn.concat(
					this.$each(vPath, (docPath, schemaPath) =>
						Promise.resolve(
							mym.valuePaths[vPath].call(this, this, _.get(this, docPath.slice(0, -1)), docPath, schemaPath)
						).then(newVal => {
	...
	var tidySchema = (node, path = [], offset, overrideSingleDef = false) => {
		...
		if (_.isArray(node)) {
			...
			if (node.index) {
				/**
				* Mutate an index
				* The return from this function is assumed to replace node.index entirely
				* @param {*} n The input node.index value
				* @returns {*} The replacement index value
				*/
				var mutateIndexNode = n => { // Mutate an index node and return
					if (n == 'primary') { // Treat as primary key
						mym.settings.idField = _.first(path.slice(-1, 2));
						return {hashKey: true};
					} else if (n == 'sort') { // Treat as default rangeKey
						return {rangeKey: true};
					} else if (n === true) { // Passthru as global
						return {global: true};
					} else if (n === false) { // Disable indexing
						// Pass through
						return false;
					} else if (_.isEqual(n, {unique: true})) { // Handle as unique hash key
						debug(`FIXME: No idea how to validate unique secondary indexes for model ${mym.id} at path ${n.path}`);
						return true;
					} else if (_.isString(n)) { // Shorthand to secondary global
						return {global: true, project: true, name: n};
					} else if (_.isPlainObject(n)) { // Pass to Dyanmoose
						// Pass thru
						return n;
					} else {
						debug(`Unknown index type in schema n ${node.path}`, n);
						throw new Error(`Unknown index type for model ${mym.id} at path ${node.path}`);
					}
				};

				if (_.isArray(node.index)) {
					node.index = node.index.map(n => mutateIndexNode(n));
				} else {
					node.index = mutateIndexNode(node.index);
				}
			}
			// }}}
			...
			if (node.value) {
				debugDetail('Allocate valuePath', mym.id, node.path);
				mym.valuePaths[node.path] = node.value;
			}
			// }}}
		...
		return Promise.resolve()
			...
			.then(data => new Promise((resolve, reject) =>
				mym.model.create(data, (err, created) => {
					if (err) {
						debug('Failed to create doc', mym.id, 'Error:', err, 'Doc:', doc);
						return reject(err);
					}
					resolve(created);
				})
	...
	*/
	mym.describe = options => new Promise((resolve, reject) => {
		...
		if (settings.cached && mym.describeCache) return resolve(mym.describeCache);

		debug(`Describe table "${id}"`);
		my.dynamoose.dynamoDB.describeTable({TableName: id}, (err, spec) => {
			if (err) return reject(err);

			// Compute moodyIndexLookup {{{
			var evalKeySchema = (schema, patch) => ({
				...patch,
				filters: schema.filter(s => s.KeyType == 'HASH').map(i => i.AttributeName),
				sorts: schema.filter(s => s.KeyType == 'RANGE').map(i => i.AttributeName),
			});

			debug('RAW SPEC', JSON.stringify(spec, null, '\t'));
			spec.moodyIndexLookup = [].concat(
				[evalKeySchema(spec.Table.KeySchema, {type: 'partitionKey', index: 'partitionKey', indexN: spec.Table.KeySchema.filter(i => i.KeyType == 'HASH')[0].AttributeName})],
				(spec.Table.LocalSecondaryIndexes || []).map(i => evalKeySchema(i.KeySchema, {type: 'localSecondary', index: i.IndexName})),
				(spec.Table.GlobalSecondaryIndexes || []).map(i => evalKeySchema(i.KeySchema, {type: 'globalSecondary', index: i.IndexName})),
			);
			// }}}

			if (settings.cached) mym.describeCache = spec;
			resolve(spec);
		});
	});
